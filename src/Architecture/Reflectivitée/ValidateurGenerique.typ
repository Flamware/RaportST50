...
#align(center)[
  #block(inset: 15pt, fill: rgb("#f4f4f4"), radius: 4pt, stroke: rgb("#dddddd"))[
    ...
    #v(0.5em)
    #text(size: 12pt, style: "italic", gray)[Pattern GenericValidator : Réflexivité au service du Clean Code]
  ]
]
...
+ #strong[Phase Statique (Initialisation) :] Lors du chargement du contexte Spring, le validateur cartographie les types génériques (`T` et `D`). Il pré-indexe les champs physiques (`Field`) et les méthodes de validation spécifiques (`@ValidationMethod`) dans des Maps.
