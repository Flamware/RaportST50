...
- *Fragilité de l'environnement frontend :* Les tests de fumée (*smoke tests*) permettant de valider l'état des routes utilisées côté frontend après déploiement étaient inexistants. Une régression sur ces routes pouvait, selon les cas, être détectée par les tests d'intégration mais plus tardivement dans le cycle de la pipeline, ou bien passer totalement inaperçue si aucun test d'intégration ne couvrait la route concernée.
