...
#h(1cm) Le département CNAPPS (Cloud, Network, Architecture, Performance, Security & Software) de Magellium est organisé en plusieurs pôles d'expertise, chacun dédié à un domaine technologique spécifique. Ces pôles sont structurés de manière à favoriser la collaboration interdisciplinaire tout en permettant une spécialisation approfondie dans les domaines clés de l'entreprise. Parmi ces pôles, on trouve notamment l'expertise en ingénierie logicielle, le pôle infrastructure et Cloud, ainsi que les unités dédiées à l'intelligence artificielle et aux systèmes autonomes.

== Organisation de l'équipe
#h(1cm) L'équipe au sein de laquelle j'ai évolué est structurée de manière à favoriser la collaboration et l'efficacité opérationnelle. Elle est composée de plusieurs membres aux compétences complémentaires, répartis selon les rôles suivants :
...
- *Ingénierie Backend et Tests* : J'ai travaillé sur la refactorisation de composants complexes en Java (via l'API de réflexion) et mené la migration complète des tests d'API vers de nouveaux standards (Bruno pour l'intégration, Playwright pour l'E2E frontend).
- *R&D en IA Agentique* : J'ai conçu de bout en bout une infrastructure locale basée sur LangGraph et le Model Context Protocol (MCP). J'ai configuré des workflows multi-agents capables d'analyser du code source, de générer des plans de tests et de rédiger des documentations de manière autonome.
...
- *Benchmark et migration d'outils* : Face aux lenteurs des tests End-to-End, j'ai initié le benchmarking de Playwright face à Cypress. Ce choix a permis de diviser le temps d'exécution par trois (de 5m28s à 1m45s) et d'améliorer la stabilité de la suite de tests.
...
- *Optimisation des ressources* : Mes initiatives sur le profilage du cache Spring Boot ont permis de réduire le temps de build local de 17 minutes à seulement 3 minutes, améliorant ainsi considérablement la vélocité de l'équipe.
...
La restitution a été une composante essentielle de ma mission. J'ai conçu et animé plusieurs présentations techniques formelles (notamment 28 slides sur la transformation "Agentic AI Farmstar" et une restitution sur l'optimisation de la CI), permettant d'évangéliser ces nouvelles pratiques architecturales auprès de l'équipe.
...
- Une démarche d'intégration continue stricte via GitLab, où chaque évolution faisait l'objet de revues de code croisées.
